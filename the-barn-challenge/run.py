...
if __name__ == "__main__":
    ...
    goal_x = GOAL_POSITION[0] * np.cos(-INIT_POSITION[2]) - GOAL_POSITION[1] * np.sin(-INIT_POSITION[2])
    goal_y = GOAL_POSITION[0] * np.sin(-INIT_POSITION[2]) + GOAL_POSITION[1] * np.cos(-INIT_POSITION[2])
    goal_theta = INIT_POSITION[2]

    cnn_rl_path = rospack.get_path('cnn_rl')
    launch_file = join(cnn_rl_path, 'launch', 'planner_launch.launch')
    ...
    # mb_goal.target_pose.pose.orientation = Quaternion(0, 0, np.sin(goal_theta / 2), np.cos(goal_theta / 2))
